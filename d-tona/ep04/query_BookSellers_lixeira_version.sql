...
from (
        select t2.seller_id,
            ...
            cast(
                julianday('2017-04-01') - julianday(max(t1.order_approved_at)) as integer
...
from (
        ...
        from tb_orders as t1
            ...
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-05-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
...
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-06-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-06-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-06-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-06-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-07-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-07-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-07-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-07-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-07-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-08-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-08-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-08-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-08-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-08-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-09-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-09-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-09-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-09-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-09-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-10-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-10-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-10-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-10-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-10-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-11-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-11-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-11-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-11-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-11-01' and '2017-11-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2017-12-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2017-12-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2017-12-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2017-12-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2017-12-01' and '2017-12-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-01-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-01-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-01-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
            ...
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-01-01' and '2018-01-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-02-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-02-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-02-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2018-02-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-02-01' and '2018-02-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-03-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-03-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-03-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2018-03-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-03-01' and '2018-03-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-04-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-04-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-04-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2018-04-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-04-01' and '2018-04-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-05-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-05-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-05-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2018-05-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-05-01' and '2018-05-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id
UNION
select '2018-06-01' as dt_ref,
    t2.seller_city,
    t2.seller_state,
    t1.*
from (
        select t2.seller_id,
            ...
            -- meses desde a primeira venda
            cast(
                julianday('2018-06-01') - julianday(max(t1.order_approved_at)) as integer
            ) as qtde_dias_utl_venda,
            count(distinct strftime('%m', t1.order_approved_at)) as qtde_mes_ativacao,
            cast(
                count(distinct strftime('%m', t1.order_approved_at)) as float
        ...
        from tb_orders as t1
            ...
            left join(
                select t2.seller_id,
                    max(
                        julianday('2018-06-01') - julianday(t1.order_approved_at)
                    ) as idade_base
                from tb_orders as t1
                    left join tb_order_items as t2 on t1.order_id = t2.order_id
                where t1.order_approved_at < '2018-06-01'
                    and t1.order_status = 'delivered'
                group by t2.seller_id
            ) as t3 on t2.seller_id = t3.seller_id
            left join tb_products as t4 on t2.product_id = t4.product_id
            left join tb_order_reviews as t5 on t1.order_id = t5.order_id
        where t1.order_approved_at between '2018-06-01' and '2018-06-01'
            and t1.order_status = 'delivered'
        group by t2.seller_id
    ) as t1
    left join tb_sellers as t2 on t1.seller_id = t2.seller_id